...
func TestCreateEvent(t *testing.T) {
	Convey("Given a MongoDB connection and events collection", t, func() {
		...
		Convey("When creating a new event with user information", func() {
			...
			links := domain.NewEventLinks("event-1", "job-123")
			event := &domain.Event{
				ID:          "event-1",
				JobID:       "job-123",
			...
			Convey("And the event links should be preserved", func() {
				...
				So(retrieved.Links.Self.HRef, ShouldEqual, "/v1/migration-jobs/job-123/events/event-1")
				So(retrieved.Links.Job, ShouldNotBeNil)
				So(retrieved.Links.Job.HRef, ShouldEqual, "/v1/migration-jobs/job-123")
			})
		...
		Convey("When creating an event without user information", func() {
			links := domain.NewEventLinks("event-2", "job-456")
			event := &domain.Event{
				ID:        "event-2",
				JobID:     "job-456",
...
func TestGetJobEvents(t *testing.T) {
	Convey("Given a MongoDB connection with events for multiple jobs", t, func() {
		...
		event1 := &domain.Event{
			...
			RequestedBy: user1,
			Links:       domain.NewEventLinks("event-1", jobID),
		}
		event2 := &domain.Event{
			ID:          "event-2",
			JobID:       jobID,
			...
			RequestedBy: user2,
			Links:       domain.NewEventLinks("event-2", jobID),
		}
		event3 := &domain.Event{
			ID:          "event-3",
			JobID:       jobID,
			...
			RequestedBy: user1,
			Links:       domain.NewEventLinks("event-3", jobID),
		}
		event4 := &domain.Event{
			ID:          "event-4",
			JobID:       jobID,
			...
			RequestedBy: user2,
			Links:       domain.NewEventLinks("event-4", jobID),
		}
		eventOtherJob := &domain.Event{
			...
			CreatedAt:   now.String(),
			RequestedBy: user1,
			Links:       domain.NewEventLinks("event-other", otherJobID),
		}
		...
		Convey("When retrieving all events for a job without pagination", func() {
			...
			Convey("Then the operation should succeed with correct data and sorting", func() {
				...
				for _, event := range retrieved {
					So(event.JobID, ShouldEqual, jobID)
			...
			Convey("And all event links should be correct", func() {
				for _, event := range retrieved {
					...
					So(event.Links.Self.HRef, ShouldContainSubstring, fmt.Sprintf("/v1/migration-jobs/%s", jobID))
					So(event.Links.Job.HRef, ShouldEqual, fmt.Sprintf("/v1/migration-jobs/%s", jobID))
				}
			})
		})
		...
		Convey("When retrieving events for a non-existent job", func() {
			retrieved, count, err := m.GetJobEvents(ctx, "non-existent-job", 10, 0)
...
func TestCountEventsByJobID(t *testing.T) {
	Convey("Given a MongoDB connection with events for multiple jobs", t, func() {
		...
		jobID := "job-count-events"
		otherJobID := "job-other-count-events"
		...
		Convey("When counting events for a job with multiple events", func() {
			count, err := m.CountEventsByJobID(ctx, jobID)
		...
		Convey("When counting events for a job with single event", func() {
			count, err := m.CountEventsByJobID(ctx, otherJobID)
		...
		Convey("When counting events for a job with no events", func() {
			count, err := m.CountEventsByJobID(ctx, "non-existent-job")
		...
		Convey("When counting events for multiple different jobs", func() {
			count1, err1 := m.CountEventsByJobID(ctx, jobID)
			count2, err2 := m.CountEventsByJobID(ctx, otherJobID)
			count3, err3 := m.CountEventsByJobID(ctx, "non-existent-job")
		...
		Convey("When counting events for the same job multiple times", func() {
			count1, err1 := m.CountEventsByJobID(ctx, jobID)
			count2, err2 := m.CountEventsByJobID(ctx, jobID)
		...
		Convey("When counting events does not affect other jobs", func() {
			// Count one job
			count1Before, _ := m.CountEventsByJobID(ctx, jobID)

			// Count other job
			countOther, _ := m.CountEventsByJobID(ctx, otherJobID)

			// Count first job again
			count1After, _ := m.CountEventsByJobID(ctx, jobID)
