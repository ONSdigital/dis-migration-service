...
func TestCreateJob(t *testing.T) {
	Convey("Given a MongoDB connection and jobs collection", t, func() {
		...
		Convey("When creating a new job", func() {
			...
			job := domain.NewJob(jobConfig, "Test Dataset Title")
		...
		Convey("When creating multiple jobs", func() {
			...
			job1 := domain.NewJob(jobConfig1, "Test Dataset Title 1")
			job2 := domain.NewJob(jobConfig2, "Test Dataset Title 2")
...
func TestGetJob(t *testing.T) {
	Convey("Given a MongoDB connection with a stored job", t, func() {
		...
		testJob := domain.NewJob(jobConfig, "Test Dataset Title")
		...
		Convey("When retrieving an existing job by ID", func() {
			retrieved, err := m.GetJob(ctx, testJob.ID)
		...
		Convey("When retrieving a non-existent job", func() {
			retrieved, err := m.GetJob(ctx, "non-existent-id")
...
func TestGetJobs(t *testing.T) {
	Convey("Given a MongoDB connection with multiple jobs in different states", t, func() {
		...
		job1 := domain.NewJob(config1, "Test Dataset Title")
		job1.State = domain.JobStateSubmitted
		job1.LastUpdated = now.Add(-3 * time.Hour)

		job2 := domain.NewJob(config2, "Test Dataset Title")
		job2.State = domain.JobStateSubmitted
		job2.LastUpdated = now.Add(-2 * time.Hour)

		job3 := domain.NewJob(config3, "Test Dataset Title")
		job3.State = domain.JobStateApproved
		job3.LastUpdated = now.Add(-1 * time.Hour)

		job4 := domain.NewJob(config4, "Test Dataset Title")
		job4.State = domain.JobStateApproved
		...
		Convey("When retrieving all jobs without state filter", func() {
			retrieved, count, err := m.GetJobs(ctx, []domain.JobState{}, 10, 0)
		...
		Convey("When retrieving jobs with a single state filter", func() {
			stateFilter := []domain.JobState{domain.JobStateSubmitted}
			...
			Convey("Then only jobs with the specified state should be returned", func() {
				...
				for _, job := range retrieved {
					So(job.State, ShouldEqual, domain.JobStateSubmitted)
				}
			})
		})

		Convey("When retrieving jobs with multiple state filters", func() {
			stateFilter := []domain.JobState{
		...
		Convey("When retrieving jobs with limit and offset", func() {
			page1, count1, err1 := m.GetJobs(ctx, []domain.JobState{}, 2, 0)
			page2, count2, err2 := m.GetJobs(ctx, []domain.JobState{}, 2, 2)
		...
		Convey("When retrieving jobs with a filter that matches nothing", func() {
			stateFilter := []domain.JobState{domain.JobStateCancelled}
			retrieved, count, err := m.GetJobs(ctx, stateFilter, 10, 0)
...
func TestGetJobsByConfigAndState(t *testing.T) {
	Convey("Given a MongoDB connection with jobs from multiple configs", t, func() {
		...
		job2 := domain.NewJob(config1, "Test Dataset Title")
		job2.State = domain.JobStateApproved
		job2.LastUpdated = now

		job3 := domain.NewJob(config1, "Test Dataset Title")
		job3.State = domain.JobStateCompleted
		job3.LastUpdated = now

		job4 := domain.NewJob(config2, "Test Dataset Title")
		job4.State = domain.JobStateSubmitted
		job4.LastUpdated = now
		...
		Convey("When retrieving jobs by config and single state", func() {
			...
			Convey("Then only jobs matching config and state should be returned", func() {
				...
				So(retrieved[0].State, ShouldEqual, domain.JobStateSubmitted)
		...
		Convey("When retrieving jobs by config with multiple states or empty filter", func() {
			// Multiple states
			retrieved1, err1 := m.GetJobsByConfigAndState(ctx, config1, []domain.JobState{
				domain.JobStateSubmitted,
				domain.JobStateApproved,
			}, 10, 0)

			// Empty filter
			retrieved2, err2 := m.GetJobsByConfigAndState(ctx, config1, []domain.JobState{}, 10, 0)
		...
		Convey("When retrieving jobs by non-matching config or with pagination", func() {
			...
			retrieved1, err1 := m.GetJobsByConfigAndState(ctx, nonExistentConfig, []domain.JobState{domain.JobStateSubmitted}, 10, 0)

			retrieved2, err2 := m.GetJobsByConfigAndState(ctx, config1, []domain.JobState{}, 1, 0)
			retrieved3, err3 := m.GetJobsByConfigAndState(ctx, config1, []domain.JobState{}, 10, 2)
...
func TestGetJobsByConfig(t *testing.T) {
	Convey("Given a MongoDB connection with jobs in multiple states for a config", t, func() {
		...
		job1 := domain.NewJob(jobConfig, "Test Dataset Title")
		job1.State = domain.JobStateSubmitted
		job1.LastUpdated = now

		job2 := domain.NewJob(jobConfig, "Test Dataset Title")
		job2.State = domain.JobStateApproved
		job2.LastUpdated = now

		job3 := domain.NewJob(jobConfig, "Test Dataset Title")
		job3.State = domain.JobStateCompleted
		job3.LastUpdated = now
		...
		Convey("When retrieving all jobs by config", func() {
			...
			Convey("Then all jobs in any state should be returned", func() {
				...
				states := make(map[domain.JobState]bool)
				for _, job := range retrieved {
