...
func TestDatasetDownloadTaskExecutor(t *testing.T) {
	Convey("Given a dataset download task executor when revert is called", t, func() {
		...
		Convey("Then it updates the task state to cancelled", func() {
			So(err, ShouldBeNil)
			...
			Convey("When migrate is called for a download task", func() {
				...
				Convey("Then no error is returned", func() {
					...
					Convey("And the UploadService is called to upload a file", func() {
						...
						So(mockUploadClient.UploadCalls()[0].Metadata.DatasetID, ShouldEqual, testDownloadTask.Target.DatasetID)
						So(mockUploadClient.UploadCalls()[0].Metadata.Edition, ShouldEqual, testDownloadTask.Target.EditionID)
						So(mockUploadClient.UploadCalls()[0].Metadata.Version, ShouldEqual, testDownloadTask.Target.VersionID)
