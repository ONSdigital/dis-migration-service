...
func (mig *migrator) CheckAndUpdateJobStateBasedOnTasks(ctx context.Context, jobID string, rule StateTransitionRule) error {
	...
	log.Info(ctx, rule.Description, logData)
	...
	log.Info(
		ctx,
		"job state updated based on task completion",
		log.Data{
			"jobID":    job.ID,
			"newState": rule.JobTargetState,
		},
	)
...
// TriggerJobStateTransitionIfComplete checks all transition rules and
// updates job state if conditions are met
func (mig *migrator) TriggerJobStateTransitionIfComplete(ctx context.Context, jobID string) error {
	...
	for _, rule := range rules {
		...
		if err != nil {
			log.Error(
				ctx,
				"error checking job state transition rule",
				err,
				log.Data{
					"jobID":           jobID,
					"taskTargetState": rule.TaskTargetState,
